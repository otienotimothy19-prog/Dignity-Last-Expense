...
export function SendQuotationForm({ quotationId, defaultEmail }: { quotationId: string; defaultEmail: string }) {
  const action = sendQuotationEmailAction.bind(null, quotationId);
  ...
  return (
    <form action={formAction} className="space-y-1.5 rounded-lg border border-imoth-grey-border p-3">
      ...
      >
        <Send className="h-4 w-4" /> {isPending ? "Sending…" : "Send Quotation"}
      </button>
    </form>
  );
}
