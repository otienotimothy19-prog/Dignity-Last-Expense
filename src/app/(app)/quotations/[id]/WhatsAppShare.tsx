...
import { logShareAttemptAction } from "./actions";

/**
 * Opens the agent's own WhatsApp with the quotation pre-filled, rather than
 * sending automatically — there's no WhatsApp Business API configured in
 * this environment. Email dropped this same "quick share" treatment once
 * real Zoho SMTP delivery landed (see sendQuotationEmailAction) — sharing a
 * mailto: link next to a working automated send was redundant and just
 * cluttered Communication History with attempts that can never resolve past
 * "Queued". wa.me can't attach a file either, so the PDF still has to be
 * attached by hand after downloading it above.
 */
export function WhatsAppShare({
  ...
  function shareWhatsApp() {
    ...
    const message = [
      `Dignity Last Expense Quotation ${referenceCode}`,
      "",
      entityName,
      planLabel,
      ...
      "",
      "The quotation certificate (PDF) is attached separately.",
    ...
    logShareAttemptAction(quotationId, "WHATSAPP", phone.trim()).catch((err) => console.error("Failed to log share attempt:", err));
  }
